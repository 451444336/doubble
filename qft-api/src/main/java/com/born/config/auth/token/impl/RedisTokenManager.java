...
public class RedisTokenManager implements TokenManager {
	...
	public Token createToken(String userId) {
		...
		try {
			...
			iCacheService.set(StringUtil.appendRedisKey(AppRedisKeyConstants.TOKEN_KEY, userId), resultToken,
					AppRedisKeyConstants.TOKEN_DAY, TimeUnit.DAYS);
	...
	public boolean checkToken(Token tokenModel) {
		...
		String token = (String) iCacheService.get(
				StringUtil.appendRedisKey(AppRedisKeyConstants.TOKEN_KEY, SecurityUtil.decryptDes(tokenModel.getUserId())));
		if (token == null || !token.equals(tokenModel.getToken())) {
			return false;
		}

		// 如果验证成功，说明此用户进行了一次有效操作，延长token的过期时间
		iCacheService.expire(StringUtil.appendRedisKey(AppRedisKeyConstants.TOKEN_KEY, tokenModel.getUserId()),
				AppRedisKeyConstants.TOKEN_EXPIRES_HOUR, TimeUnit.HOURS);
		return true;
	}
