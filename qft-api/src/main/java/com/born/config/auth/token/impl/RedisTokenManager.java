...
public class RedisTokenManager implements TokenManager {
	...
	public Token createToken(String userId) {
		...
		try {
			...
			iCacheService.set(AppConstants.TOKEN_KEY + ":" + userId, resultToken, AppConstants.TOKEN_DAY,
					TimeUnit.DAYS);
	...
	public boolean checkToken(Token tokenModel) {
		...
		String token = (String) iCacheService
				.get(AppConstants.TOKEN_KEY + ":" + SecurityUtil.decryptDes(tokenModel.getUserId()));
		if (token == null || !token.equals(tokenModel.getToken())) {
			return false;
		}

		// 如果验证成功，说明此用户进行了一次有效操作，延长token的过期时间
		iCacheService.expire(AppConstants.TOKEN_KEY + ":" + tokenModel.getUserId(), AppConstants.TOKEN_EXPIRES_HOUR,
				TimeUnit.HOURS);
		return true;
	}
