...
public class PermissionApi {
	...
	* @throws
	 */
	@ApiOperation(value = "权限编辑",notes = "[李杰]权限编辑")
    @ApiResponses(value = {
            @ApiResponse(code = 10100,message = "请求参数有误"),
            @ApiResponse(code = 200,message = "操作成功"),
            @ApiResponse(code = 300,message = "操作失败"),
	...
	@PutMapping("anon")
	@LimitIPRequest
	public Result updatePermission(@RequestBody PermissionInfoDTO dto) {
		log.info("权限编辑请求参数={}", JSON.toJSONString(dto));
		Result result = permissionService.updatePermission(dto);
		log.info("权限编辑返回数据={}", JSON.toJSONString(result));
		return result;
	}
}
