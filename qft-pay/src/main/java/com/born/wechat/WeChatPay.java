package com.born.wechat;

import java.util.HashMap;
import java.util.Map;

import com.born.config.wechat.WXPayConfig;
import com.born.util.WXPayUtil;
 ...
 * 
 * @ClassName: WXPay
 * @Description: 微信支付调用入口，可根据自己需要进行实例化
 * @author: lijie
 * @date: 2018年1月29日 下午2:01:38
 */
@Slf4j
public class WeChatPay {
    ...
    public WeChatPay(final WXPayConfig config) throws Exception {
        this(config, null, true, false);
    }

    public WeChatPay(final WXPayConfig config, final boolean autoReport) throws Exception {
        this(config, null, autoReport, false);
    }


    public WeChatPay(final WXPayConfig config, final boolean autoReport, final boolean useSandbox) throws Exception{
        this(config, null, autoReport, useSandbox);
    }

    public WeChatPay(final WXPayConfig config, final String notifyUrl) throws Exception {
        this(config, notifyUrl, true, false);
    }

    public WeChatPay(final WXPayConfig config, final String notifyUrl, final boolean autoReport) throws Exception {
        this(config, notifyUrl, autoReport, false);
    }

	public WeChatPay(final WXPayConfig config, final String notifyUrl, final boolean autoReport,
